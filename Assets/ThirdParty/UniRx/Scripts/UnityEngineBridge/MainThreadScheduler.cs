...
namespace UniRx
{
    ...
    public static partial class Scheduler
    {
        ...
        private static IScheduler mainThreadRealtime;

        public static IScheduler MainThreadRealtime => mainThreadRealtime ?? (mainThreadRealtime = new RealtimeMainThreadScheduler());
        ...
        class RealtimeMainThreadScheduler : IScheduler, ISchedulerPeriodic, ISchedulerQueueing
        {
           readonly Action<object> scheduleAction;
            ...
            IEnumerator DelayAction(TimeSpan dueTime, Action action, ICancelable cancellation)
            {
                ...
                {
                    var dueTimeTotalSeconds = Time.realtimeSinceStartup + dueTime.TotalSeconds;
                    while (true)
                    {
                        yield return null;
                        if (cancellation.IsDisposed) break;
                        
                        if (Time.realtimeSinceStartup >= dueTimeTotalSeconds)
                        {
                            MainThreadDispatcher.UnsafeSend(action);
                            break;
                        }
            ...
            IEnumerator PeriodicAction(TimeSpan period, Action action, ICancelable cancellation)
            {
                ...
                {
                    var dueTimeTotalSeconds = Time.realtimeSinceStartup + period.TotalSeconds;
                    while (true)
                    {
                        yield return null;
                        if (cancellation.IsDisposed) break;
                        ...
                        {
                            MainThreadDispatcher.UnsafeSend(action);
                            dueTimeTotalSeconds += period.TotalSeconds;
                        }
                    }
                }
            }
