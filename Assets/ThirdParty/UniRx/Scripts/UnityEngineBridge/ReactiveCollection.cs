...
namespace UniRx
{
	...
	public interface IReactiveCollection<T> : IList<T>, IReadOnlyReactiveCollection<T>, IReinitializableCollection
	{
		new int Count { get; }
	...
	[Serializable]
	public partial class ReactiveCollection<T> : Collection<T>, IReactiveCollection<T>, IReinitializedLockedCollection, IDisposable
	{
		...
		{
			if (disposedValue) return Observable.Empty<Unit>();
		...
		IEnumerable<IDisposable> IReinitializedLockedCollection.Disposables => Items.OfType<IDisposable>();

		public ReactiveCollectionReinitializeLock Reinitialize(bool disposeValues = true)
		{
			return ReactiveCollectionReinitializeLock.Reinitialize(this);
		}
		...
		{
			if (disposedValue) return Observable.Empty<int>();
		...
		{
			if (disposedValue) return Observable.Empty<CollectionAddEvent<T>>();
		...
		{
			if (disposedValue) return Observable.Empty<CollectionMoveEvent<T>>();
		...
		{
			if (disposedValue) return Observable.Empty<CollectionRemoveEvent<T>>();
		...
		{
			if (disposedValue) return Observable.Empty<CollectionReplaceEvent<T>>();
