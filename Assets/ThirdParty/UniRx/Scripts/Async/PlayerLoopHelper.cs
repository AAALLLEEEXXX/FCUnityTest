    ...
    public static class PlayerLoopHelper
    {
        ...
        private static void OnEnteredEditMode()
        {
            ...
            for (var i = 0; i < 7; i++)
            {
                ...
                yielders[i] = new ContinuationQueue();
            }

            // Update вызывается из основного потока!
            EditorApplication.update += UpdateAllRunnersInEditor;
        }
        ...
        private static void UpdateAllRunnersInEditor()
        {
            for (var i = 0; i < 7; i++)
            {
                runners[i].Run();
