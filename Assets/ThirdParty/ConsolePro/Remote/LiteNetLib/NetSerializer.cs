		...
		private StructInfo RegisterInternal<T>() where T : class
		{
			...
			for (var i = 0; i < props.Length; i++)
			{
				...
				if (isEnum)
				{
					...
					{
						info.ReadDelegate[i] = reader => { property.SetValue(info.Reference, Enum.ToObject(propertyType, reader.GetByte()), null); };
						info.WriteDelegate[i] = writer => { writer.Put((byte) property.GetValue(info.Reference, null)); };
					}
					else if (underlyingType == typeof(int))
					{
						info.ReadDelegate[i] = reader => { property.SetValue(info.Reference, Enum.ToObject(propertyType, reader.GetInt()), null); };
				...
				else
				{
					...
					if (_registeredCustomTypes.TryGetValue(propertyType, out registeredCustomType))
					{
						...
						{
							info.ReadDelegate[i] = reader => { property.SetValue(info.Reference, registeredCustomType.ReadDelegate(reader), null); };

							info.WriteDelegate[i] = writer => { registeredCustomType.WriteDelegate(writer, property.GetValue(info.Reference, null)); };
						}
					}
					else
